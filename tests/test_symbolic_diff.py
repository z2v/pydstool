...
def test_complex():
    ...
    assert str(Diff(3 + t * f(s, t), t).eval(s=3,
                                             t=1,
                                             p=p())) == '-4.914463076812332'
    ...
    test1 = ['x', 'a*x', 'a', 'a(x)', 'a(x)*x', 'a(x)*b(x)', 'a+x', 'a(x)+x',
    ...
    sols1 = ['1', 'a', '0', 'a_0(x)', 'a_0(x)*x+a(x)',
             'a_0(x)*b(x)+a(x)*b_0(x)', '1', 'a_0(x)+1', 'a_0(x)+b_0(x)+c_0(x)',
             ...
             '(Log(a(x))*b_0(x)+a_0(x)*b(x)/(a(x)))*Pow(a(x),b(x))',
             'n*Pow(x,n-1)', '5*Pow(x,4)', '-5*Pow(x,-6)', 'Cos(x)', '-Sin(x)',
             'Exp(x)',  # '1/x', '1/x', 'Log(10)/x', '1/Sqrt(1-Pow(x,2))',
             '1.0/x', '1.0/x', 'Log(10)/x', '1.0/Sqrt(1-Pow(x,2))', 'Cosh(x)']
    for i, s in enumerate(test1):
        dstr = str(Diff(s, x))
        if dstr != sols1[i]:
            raise 'Test %d: Diff(%s,%s) = %s but should be %s' % (i, s, x,
                                                                  dstr,
                                                                  sols1[i])
    ...
    for i in range(2):
        ...
        if dstr != sols2[i]:
            raise 'Diff(%s,%s) = %s but should be %s' % (str1, 'x', dstr,
                                                         sols2[i])
        str1 = dstr
    ...
    if dstr2 != '-1.5*Pow(x,-2.5)':
        raise 'Diff(%s,%s)=%s but should be %s' % (str2, 'x', dstr2,
                                                   '-1.5*Pow(x,-2.5)')
